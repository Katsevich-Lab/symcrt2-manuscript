...
sim_name <- "oat_pcm"
sim_spec_fp <- paste0("code/sim_spec/hmm_interacted_computation/sim_spec_", sim_name, ".rds")

# Create the parameter grid
# The default values for the simulation
seed = 1234
baseline_values = list(n = 4000,  # sample size
                       p = 250,  # dimension of Z
                       s = 20,
                       ...
                       amplitude = 0.80)
varying_values = list(n = c(4000),  # sample size
                      p = seq(200, 300, 25),  # dimension of Z
                      s = c(20),
                      joint_X = "HMM", y_given_X = "interacted", 
...
method_strings <- c(
  "oat_pcm ranger_categorical ranger 0.45 FALSE BH NA FALSE FALSE"
)


# Generate the simulatr specified object
simulatr_spec <- symcrt2::create_simspec_object_parameter_grid(alpha = 0.1, seed = 4, B = 3,
