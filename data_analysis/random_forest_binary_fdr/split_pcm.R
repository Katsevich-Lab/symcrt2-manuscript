...
for (prop in proportions) {
  for (i in 1:iter) {
    start_time <- Sys.time()
    split_pcm_placeholder <- symcrt2::split_pcm(data = data, fit_X = symcrt2::fit_X_CV_graphical_lasso,
                                       training_proportion = prop, linear = FALSE, B = 25,
                                       fit_y_given_X = symcrt2::fit_y_given_X_ranger_binary,
                                       alpha = alpha, degeneracy_correction = FALSE,
                                       multiple_correction = "BH", seed = i)
    end_time <- Sys.time()
    split_pcm_placeholder$time <- end_time - start_time
    results[[as.character(prop), i]] <- split_pcm_placeholder
  }
}
